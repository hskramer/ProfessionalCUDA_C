...
float cuda_device_rand()
{
	...
	if (dRand == NULL)
	{
		...
		*/
		...
		initialize_state <<<blocks_per_grid, threads_per_block, 0, stream >>>(states);
	...
	if (dRand_used == dRand_length)
	{
		...
		*/
		...
		refill_randoms <<<blocks_per_grid, threads_per_block, 0, stream >>>(dRand, dRand_length, states);
		dRand_used = 0;
	}
...
float cuda_host_rand()
{
	...
	if (dRand_used == dRand_length)
	{
		/*
		* If all pre-generated random numbers have been consumed, wait for the
		* last asynchronous curandGenerateUniform to complete, transfer the new
